...
Start with `python scripts/hh.py demo`, then `python -m unittest discover -s tests -v`.
...
One business rule lives in `workflows/` or `methodology/`, not several subtly different host prompts.

...
from household_cio.core import InputError,cashflow,allocation,snapshot,stress
...
class ArtifactTests(unittest.TestCase):
    def test_skills_have_frontmatter_and_workflow(self):
        ...
        for p in paths:
            ...
            self.assertTrue((ROOT/'workflows'/f'{p.parent.name.removeprefix("hh-")}.md').exists())
    ...
    def test_registry_truth(self):
        implemented=[p['id'] for p in read_json(ROOT/'providers/registry.json')['providers'] if p['status']=='implemented']
        self.assertIn('manual-json',implemented);self.assertIn('cashflow-csv',implemented)
    def test_no_network_import_in_engine(self):
        for p in (ROOT/'household_cio').glob('*.py'):
            self.assertNotRegex(p.read_text(),r'(?m)^\s*(?:from|import)\s+(?:requests|httpx|urllib|socket|aiohttp)\b')
    def test_schema_loads(self):
        schema=read_json(ROOT/'schemas/household.schema.json');self.assertIn('$schema',schema);self.assertIs(schema['additionalProperties'],False)
    ...
    def test_readme_does_not_claim_live_adapters(self):
        self.assertIn('No live market feeds in the bundled demo',(ROOT/'README.md').read_text())

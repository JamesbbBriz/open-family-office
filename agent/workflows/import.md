...
2. Build a proposed mapping to `agent/schemas/household.schema.json`. Transactions and balances are different.
...
4. Cashflow CSV imports use exactly the header in `agent/templates/cashflows.csv`. They append to a new model:
   `python scripts/ofo.py import-csv <csv> --household <old.json> --out <new-private.json>`.

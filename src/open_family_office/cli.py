...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Open Family Office: local-first household wealth research tools; no trading or autonomous advice.")
    sub = p.add_subparsers(dest="command", required=True)
    init = sub.add_parser("init", help="Create an empty PRIVATE workspace outside this repo")
    init.add_argument("--workspace", required=True)
    for command in ["validate", "snapshot", "cashflow", "stress", "compare-allocation", "overview"]:
        ...
        if command == "stress":
            ...
            sp.add_argument("--scenario")
        elif command == "compare-allocation":
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "init":
            ...
            template = read_json(REPO_ROOT / "agent/templates/household.json")
        ...
        if args.command == "import-csv":
            ...
            with Path(args.csv_path).expanduser().open(encoding="utf-8-sig", newline="") as stream:
                ...
                if set(reader.fieldnames or []) != expected or len(reader.fieldnames or []) != len(expected):
                    raise InputError("CSV columns must exactly match agent/templates/cashflows.csv")
                rows = list(reader)
            if not rows:

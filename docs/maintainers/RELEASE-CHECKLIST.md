...
Run the clean-checkout tests, dashboard builder and `python scripts/release_check.py`. Compare [validation](../VALIDATION.md) with any claims in README, release notes, screenshots and CV entries. Optional skipped tests must remain visible; a local run is not a green remote GitHub Actions badge.

...
Collect a user-confirmed acyclic ownership graph with unique node IDs, explicit percentage edges, base-currency leaf values and types. Never infer legal beneficial ownership. Run `python scripts/ofo.py lookthrough <file>`. Reject parent-plus-child valuation duplication, cycles, invalid shares or ambiguous roots. Reconcile leaf totals to separately supplied household positions before using the result. For exposure inputs use `python scripts/ofo.py exposures <file>`; unknown fractions stay unknown and different dimensions are not additive. State valuation dates and evidence gaps. No synthetic beta, private-asset correlation or automatic property valuation.
